...
function getToken(req, instance) {
    var token, cookies = {};
    req.headers.cookie.split(';').forEach(function(cookie) {
        var parts = cookie.split('=');
        cookies[parts[0].trim()] = (parts[1] || '').trim();
    });
    if (cookies[instance._openAmCookieName]) {
        token = cookies[instance._openAmCookieName]
    }
    return token;
}
...
/**
 * Logout request by delegating to a service provider using OpenAM.
 *
 * @param {Object} req
 * @api protected
 */
OpenAmStrategy.prototype.logout = function(req, callback) {
    var token = getToken(req, this);
    if (token) {
        this._openam.logout(token, function(error, data) {
            callback(error, data);
        });
    }
}
        ...
        if (token) {
            this._openam.isTokenValid(token, function(bool) {
                ...
                if (bool) {
                    ...
                    var location = self._openam.getLoginUiUrl(params);
...
OpenAmStrategy.prototype.userProfile = function(token, done) {
    this._openam.getAttributes(token, function(err, data) {
        if (err) {
            return done(new InternalOpenAmError('failed to get attributes', err));
        }
        try {
            var profile = {};
